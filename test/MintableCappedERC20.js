...
describe('MintableCappedERC20', () => {
  ...
  describe('burning from account with given approve', () => {
    it('should burnFrom address', async () => {
      ...
      await expect( await token.connect(userWallet).approve(spender, amount))
        .to.emit(token, 'Approval').withArgs(issuer, spender, amount)

      await expect(await token.burnFrom(issuer, amount))
        .to.emit(token, 'Transfer').withArgs(issuer, ADDRESS_ZERO, amount)
    });
  })
  describe('EIP-2612: approve permit', () => {
    it('should should set allowance by verifying permit', async () => {
      ...
      await expect( await token.permit(issuer, spender, amount, deadline, signature.v, signature.r, signature.s))
        .to.emit(token, 'Approval').withArgs(issuer, spender, amount)

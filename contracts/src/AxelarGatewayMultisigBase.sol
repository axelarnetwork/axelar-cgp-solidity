...
abstract contract AxelarGatewayMultisigBase is AxelarGateway {
    ...
    function _owners() internal view returns (address[] memory results) {
        uint256 ownerEpoch = _ownerEpoch();
    ...
    function _operators() internal view returns (address[] memory results) {
        uint256 operatorEpoch = _operatorEpoch();
    ...
    function _execute(bytes memory data, bytes[] memory signatures) internal {
        ...
        for (uint256 i; i < commandsLength; i++) {
            ...
            if (commandHash == SELECTOR_DEPLOY_TOKEN) {
                commandSelector = IAxelarGatewayMultisig.deployToken.selector;
            } else if (commandHash == SELECTOR_MINT_TOKEN) {
                commandSelector = IAxelarGatewayMultisig.mintToken.selector;
            } else if (commandHash == SELECTOR_BURN_TOKEN) {
                commandSelector = IAxelarGatewayMultisig.burnToken.selector;
            } else if (commandHash == SELECTOR_TRANSFER_OWNERSHIP) {
                commandSelector = IAxelarGatewayMultisig.transferOwnership.selector;
            } else if (commandHash == SELECTOR_TRANSFER_OPERATORSHIP) {
                commandSelector = IAxelarGatewayMultisig.transferOperatorship.selector;
            } else {
